...
test.describe("authenticated shell", () => {
  test.skip(!authenticatedState, "Set PLAYWRIGHT_AUTH_STORAGE_STATE to an authenticated local-Supabase storage state.");

  test("keeps Inbox reachable and makes the capture shortcut focus its composer", async ({ page }) => {
    await page.goto("/inbox");
    await expect(page.getByRole("heading", { name: "Nothing important slips through." })).toBeVisible();
    await page.keyboard.press("ControlOrMeta+J");
    await expect(page.locator("#capture")).toBeFocused();
  });

  test("routes transparent build-state pages from primary and More navigation", async ({ page }) => {
    await page.goto("/inbox");
    await page.getByRole("link", { name: "Today" }).click();
    await expect(page.getByRole("heading", { name: "A quieter daily view is taking shape." })).toBeVisible();
    await page.getByText("More", { exact: true }).first().click();
    await page.getByRole("link", { name: "People & Notes" }).first().click();
    await expect(page.getByRole("heading", { name: "Personal context comes later, on purpose." })).toBeVisible();
  });
});
