...
export async function POST(request: NextRequest, context: { params: Promise<{ signalId: string }> }) {
  ...
  if (parsed.data.outcome === "cadence_changed") {
    const { data: existing } = await supabase.from("slipping_signals").select("entity_type, entity_id").eq("id", signalId).maybeSingle();
    if (!existing) return badRequest("Signal not found.");
    if (existing.entity_type !== "task" && existing.entity_type !== "project") return badRequest("Cadence can only be changed for tasks and projects.");
    const table = existing.entity_type === "task" ? "tasks" : "projects";
    const { error: cadenceError } = await supabase.from(table).update({ slipping_cadence_days: parsed.data.cadenceDays }).eq("id", existing.entity_id);
  ...
  const { error: activityError } = await supabase.from("activity_events").insert({
    ...
    entity_id: signal.entity_type === "retainer_cycle_item" && signal.retainer_id ? signal.retainer_id : signal.entity_id,
