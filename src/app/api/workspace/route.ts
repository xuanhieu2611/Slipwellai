...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) {
    ...
    return badRequest(
      field
        ? `That workspace change was not valid (${field}).`
        : "That workspace change was not valid.",
    );
  }
  ...
  async function verifyRelations(
    values: Partial<Record<keyof typeof relationTables, string | null | undefined>>,
  ) {
    for (const [key, table] of Object.entries(relationTables) as Array<
      [keyof typeof relationTables, (typeof relationTables)[keyof typeof relationTables]]
    >) {
  ...
  async function recordActivity(
    ...
    const { error } = await supabase
      ...
      .insert({ entity_type: entityType, entity_id: entityId, event_type: eventType, metadata });
    ...
    if (command.action === "create_domain") {
      ...
      const { data: task, error } = await supabase
        ...
        .insert({
          ...
          due_on: command.dueOn ?? null,
          scheduled_for: command.scheduledFor ?? null,
          priority: command.priority,
          recurrence_rule: recurrenceRule,
          recurrence_anchor: recurrenceRule ? command.scheduledFor : null,
          recurrence_interval: recurrenceRule === "custom" ? command.recurrenceInterval : null,
          recurrence_unit: recurrenceRule === "custom" ? command.recurrenceUnit : null,
          tags: command.tags,
          domain_id: command.domainId ?? null,
          project_id: command.projectId ?? null,
          person_id: command.personId ?? null,
          retainer_id: command.retainerId ?? null,
      ...
      if (recurrenceRule) {
        const { error: rootError } = await supabase
          .from("tasks")
          .update({ recurrence_root_id: task.id })
          .eq("id", task.id);
        if (rootError) throw rootError;
      }
      if (command.projectId)
        await recordActivity("project", command.projectId, "task_created", { taskId: task.id });
      ...
      const { error } = await supabase
        .from("tasks")
        .update({ archived_at: command.action === "delete_task" ? new Date().toISOString() : null })
        .eq("id", command.taskId);
      if (error) throw error;
      await recordActivity(
        "task",
        command.taskId,
        command.action === "delete_task" ? "deleted" : "restored",
      );
      ...
      if (openTasks.error || activeProjects.error || people.error || notes.error)
        throw openTasks.error ?? activeProjects.error ?? people.error ?? notes.error;
      const counts = {
        tasks: openTasks.count ?? 0,
        projects: activeProjects.count ?? 0,
      ...
      if (counts.tasks + counts.projects + counts.people + counts.notes > 0) {
        ...
        if (counts.projects)
          parts.push(`${counts.projects} active project${counts.projects === 1 ? "" : "s"}`);
        if (counts.people)
          parts.push(`${counts.people} linked ${counts.people === 1 ? "person" : "people"}`);
        if (counts.notes) parts.push(`${counts.notes} linked note${counts.notes === 1 ? "" : "s"}`);
        return badRequest(
          `This domain still has ${parts.join(", ")}. Reassign or resolve them before archiving.`,
        );
      }
      const { error } = await supabase
        .from("domains")
      ...
      const { data: project, error } = await supabase
        ...
        .insert({
          ...
          domain_id: command.domainId ?? null,
          person_id: command.personId ?? null,
          start_on: command.startOn ?? null,
          target_on: command.targetOn ?? null,
          slipping_cadence_days: command.slippingCadenceDays ?? null,
          idempotency_key: command.idempotencyKey,
        })
        .select("id")
      ...
      const { error } = await supabase
        .from("projects")
        .update({
          archived_at: command.action === "delete_project" ? new Date().toISOString() : null,
        })
        .eq("id", command.projectId);
      if (error) throw error;
      await recordActivity(
        "project",
        command.projectId,
        command.action === "delete_project" ? "deleted" : "restored",
      );
      ...
      const { error } = await supabase
        ...
        .insert({ name: command.name, description: command.description });
      if (error) throw error;
      ...
      const { error } = await supabase
        .from("project_checklist_template_items")
        .insert({ template_id: template.id, title: command.title, position: (count ?? 0) + 1 });
      if (error) throw error;
      const { error: versionError } = await supabase
        .from("project_checklist_templates")
        .update({ version: template.version + 1 })
        .eq("id", template.id);
      ...
      if (command.applyToExisting) {
        const { error: existingError } = await supabase
          ...
          .update({ title: command.title })
          .eq("source_template_item_id", item.id)
          .eq("status", "open");
      ...
      const [templateResult, projectResult, itemsResult] = await Promise.all([
        ...
        supabase
          ...
          .eq("template_id", command.templateId)
          .is("archived_at", null)
          .order("position"),
      ]);
      const template = templateResult.data;
      if (!template || !projectResult.data)
        return badRequest("Project or checklist template not found.");
      if (itemsResult.error) throw itemsResult.error;
      if (!itemsResult.data?.length)
        return badRequest("Add at least one checklist item before applying this template.");
      const { error: instanceInsertError } = await supabase
        ...
        .upsert(
          ...
          { onConflict: "project_id,template_id,template_version", ignoreDuplicates: true },
        );
      ...
      if (instanceError || !instance)
        throw instanceError ?? new Error("Checklist instance not found.");
      const { error: itemError } = await supabase.from("project_checklist_items").upsert(
        ...
        { onConflict: "instance_id,source_template_item_id", ignoreDuplicates: true },
      );
      ...
      const { error } = await supabase
        .from("people")
        .update({
          archived_at: command.action === "delete_person" ? new Date().toISOString() : null,
        })
        .eq("id", command.personId);
      if (error) throw error;
      ...
      if (command.followUpTitle) {
        const { data: task, error: taskError } = await supabase
          .from("tasks")
          .insert({ title: command.followUpTitle, person_id: person.id })
          .select("id")
          .single();
      ...
      const { error } = await supabase.from("notes").insert({
        ...
        project_id: command.projectId ?? null,
        person_id: command.personId ?? null,
        review_on: command.reviewOn ?? null,
      });
      if (error) throw error;
      ...
      const { error } = await supabase
        .from("notes")
        .update({ archived_at: command.action === "delete_note" ? new Date().toISOString() : null })
        .eq("id", command.noteId);
      if (error) throw error;
    } else if (command.action === "create_routine") {
      const { error } = await supabase
        .from("routines")
        .insert({ name: command.name, period: command.period });
      if (error) throw error;
    } else if (
      command.action === "complete_task" ||
      command.action === "reopen_task" ||
      command.action === "defer_task" ||
      command.action === "cancel_task"
    ) {
      const { data: task } = await supabase
        .from("tasks")
        .select(
          "id, title, details, priority, due_on, scheduled_for, recurrence_rule, recurrence_root_id, recurrence_anchor, recurrence_interval, recurrence_unit, domain_id, project_id, person_id, source_capture_id",
        )
        .eq("id", command.taskId)
        .maybeSingle();
      if (!task) return badRequest("Task not found.");
      if (command.action === "complete_task" && task.recurrence_rule && task.recurrence_anchor) {
        ...
        const custom =
          task.recurrence_interval && task.recurrence_unit
            ? { interval: task.recurrence_interval, unit: task.recurrence_unit as "days" | "weeks" }
            : undefined;
        ...
        const { data: existing, error: existingError } = await supabase
          .from("tasks")
          .select("id")
          .eq("recurrence_root_id", rootId)
          .eq("recurrence_anchor", nextAnchor)
          .maybeSingle();
        if (existingError) throw existingError;
        if (!existing) {
          const { error: nextError } = await supabase.from("tasks").insert({
            ...
            due_on: task.due_on ? nextRecurrenceDate(task.due_on, rule, custom) : null,
            scheduled_for: task.scheduled_for
              ? nextRecurrenceDate(task.scheduled_for, rule, custom)
              : null,
            recurrence_rule: rule,
            recurrence_root_id: rootId,
      ...
      const changes =
        command.action === "complete_task"
          ? { status: "completed", completed_at: new Date().toISOString(), deferred_until: null }
          : command.action === "reopen_task"
            ? { status: "open", completed_at: null }
            : command.action === "cancel_task"
              ? { status: "canceled", deferred_until: null }
              : { status: "open", deferred_until: command.until ?? null };
      ...
      const { count } = await supabase
        .from("tasks")
        .select("id", { count: "exact", head: true })
        .eq("top_three_date", command.localDate);
      if ((count ?? 0) >= 3)
        return badRequest("Today already has three priorities. Remove one first.");
      const { error } = await supabase
        .from("tasks")
        .update({ top_three_date: command.localDate, top_three_order: (count ?? 0) + 1 })
        .eq("id", command.taskId);
      if (error) throw error;
    } else if (command.action === "clear_top_three") {
      const { error } = await supabase
        .from("tasks")
        .update({ top_three_date: null, top_three_order: null })
        .eq("id", command.taskId);
      if (error) throw error;
      ...
      if (
        command.taskIds.length !== currentIds.size ||
        !command.taskIds.every((taskId) => currentIds.has(taskId))
      ) {
      ...
      const { error: clearError } = await supabase
        .from("tasks")
        .update({ top_three_date: null, top_three_order: null })
        .in("id", command.taskIds);
      if (clearError) throw clearError;
      for (const [index, taskId] of command.taskIds.entries()) {
        const { error } = await supabase
          .from("tasks")
          .update({ top_three_date: command.localDate, top_three_order: index + 1 })
          .eq("id", taskId);
        if (error) throw error;
      }
      ...
      const { error } = await supabase
        .from("project_milestones")
        .update(
          command.action === "complete_milestone"
      ...
      await recordActivity(
        ...
        command.action === "complete_milestone" ? "milestone_completed" : "milestone_reopened",
      );
    } else if (
      command.action === "complete_checklist_item" ||
      command.action === "reopen_checklist_item"
    ) {
      const { data: item } = await supabase
        .from("project_checklist_items")
      ...
      if (command.scope === "future" || command.scope === "both") {
        const { error } = await supabase
          ...
          .update({
            title: command.title,
            expected_day: command.expectedDay,
      ...
      if (command.scope === "current" || command.scope === "both") {
        const { data: latestCycle } = await supabase
          ...
          .select("id")
          .eq("retainer_id", item.retainer_id)
      ...
      await recordActivity("retainer", item.retainer_id, "template_item_updated", {
        scope: command.scope,
      });
      ...
      command.action === "close_retainer_cycle_item" ||
      command.action === "leave_retainer_cycle_item_in_prior_cycle"
    ) {
      const { data: item } = await supabase
        .from("retainer_cycle_items")
      ...
      if (item.status !== "open")
        return badRequest("Only an open deliverable can be resolved this way.");
      ...
      const changes =
        command.action === "close_retainer_cycle_item"
          ...
          : { excluded_from_carry_forward: true };
      const { error } = await supabase
        .from("retainer_cycle_items")
        .update(changes)
        .eq("id", item.id);
      ...
      await recordActivity(
        ...
        command.action === "close_retainer_cycle_item"
          ? "cycle_item_closed"
          : "cycle_item_left_in_prior_cycle",
        { cycleItemId: item.id },
      );
      ...
      const { data: retainer } = await supabase
        .from("retainers")
        .select("id, status")
        .eq("id", command.retainerId)
        .maybeSingle();
      if (!retainer) return badRequest("Retainer not found.");
      const requiredStatus = command.action === "pause_retainer" ? "active" : "paused";
      if (retainer.status !== requiredStatus)
        return badRequest(
          command.action === "pause_retainer"
            ? "Only an active retainer can be paused."
            : "Only a paused retainer can be resumed.",
        );
      ...
      const { error } = await supabase
        .from("retainers")
        .update({ status: command.action === "pause_retainer" ? "paused" : "active" })
        .eq("id", retainer.id);
      if (error) throw error;
      await recordActivity(
        "retainer",
        retainer.id,
        command.action === "pause_retainer" ? "paused" : "resumed",
      );
      ...
      const { error } = await supabase
        ...
        .eq("id", retainer.id);
      if (error) throw error;
      await recordActivity("retainer", retainer.id, "ended", {
        openItemResolution: command.openItemResolution,
      });
      ...
      const { data: retainer, error } = await supabase
        ...
        .insert({
          ...
          cycle_day: command.cycleDay,
          client_person_id: command.clientPersonId ?? null,
          domain_id: command.domainId ?? null,
          idempotency_key: command.idempotencyKey,
        })
        .select("id")
      ...
      const { count, error: countError } = await supabase
        ...
        .is("archived_at", null);
      if (countError) throw countError;
      const { error } = await supabase.from("retainer_deliverable_templates").insert({
      ...
      const { error } = await supabase
        .from("retainers")
        .update({
          archived_at: command.action === "delete_retainer" ? new Date().toISOString() : null,
        })
        .eq("id", command.retainerId);
      if (error) throw error;
      await recordActivity(
        "retainer",
        command.retainerId,
        command.action === "delete_retainer" ? "deleted" : "restored",
      );
    } else if (
      command.action === "complete_retainer_cycle_item" ||
      command.action === "reopen_retainer_cycle_item"
    ) {
      const { data: item } = await supabase
        .from("retainer_cycle_items")
        .select("id, cycle_id")
      ...
      const { data: result, error: generateError } = (await supabase
        ...
        .select()
        .maybeSingle()) as { data: { out_cycle_id: string } | null; error: unknown };
      if (generateError || !result) throw generateError ?? new Error("Cycle generation failed.");

      await recordActivity("retainer", retainer.id, "cycle_generated", {
        cycleId: result.out_cycle_id,
      });
    } else if (
      command.action === "record_project_progress" ||
      command.action === "pause_project" ||
      command.action === "complete_project"
    ) {
      const { data: project } = await supabase
        .from("projects")
        .select("id")
      ...
      if (command.action === "complete_project") {
        const { count } = await supabase
          ...
          .eq("status", "open");
        if ((count ?? 0) > 0)
          return badRequest("Resolve this project’s open tasks before completing it.");
        ...
        if (instances?.length) {
          ...
          if ((checklistCount ?? 0) > 0)
            return badRequest("Resolve this project’s open checklist items before completing it.");
        }
      }
      if (command.action !== "record_project_progress") {
        const { error } = await supabase
          .from("projects")
          .update({ status: command.action === "pause_project" ? "paused" : "completed" })
          .eq("id", project.id);
        if (error) throw error;
      }
      const eventType =
