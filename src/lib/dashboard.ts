...
export const newestProposalByCapture = <T extends { capture_id: string }>(proposals: T[]) => {
  ...
  for (const proposal of proposals) {
    if (!proposalByCapture.has(proposal.capture_id))
      proposalByCapture.set(proposal.capture_id, proposal);
  }
...
export function filedDestinationLabel(
  record: Pick<RoutedRecord, "domain_id" | "project_id" | "person_id">,
  catalog: DestinationCatalog,
) {
  const names = [
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const [
    ...
    supabase
      .from("captures")
      .select(
        "id, original_text, source_type, status, failure_code, interpretation_claimed_at, created_at",
      )
      .order("created_at", { ascending: false })
      .limit(12),
    supabase
      .from("proposals")
      .select("id, capture_id, status, proposal_json")
      .order("created_at", { ascending: false }),
    supabase
      .from("proposal_applications")
      .select("proposal_id, item_index, outcome, record_type, record_id"),
    supabase
      .from("prototype_records")
      .select("id, proposal_id, record_type, title, destination_name, created_at")
      .order("created_at", { ascending: false })
      .limit(8),
    ...
    supabase
      .from("tasks")
      .select("id, proposal_id, title, created_at, domain_id, project_id, person_id")
    ...
    supabase
      ...
      .select("id, entity_type, entity_id, retainer_id, cycle_item_id, reason, severity, outcome")
      .eq("outcome", "open")
      .order("created_at", { ascending: false }),
