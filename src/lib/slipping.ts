...
export function coreSlippingExplanation(entity: CoreSlippingEntity, timezone: string, now = new Date()) {
  ...
  const elapsed = Math.max(0, Math.floor((Date.parse(`${today}T00:00:00Z`) - Date.parse(`${reference.slice(0, 10)}T00:00:00Z`)) / 86_400_000));
