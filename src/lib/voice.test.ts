...
import {
  createVoiceCaptureSchema,
  maxVoiceCaptureBytes,
  maxVoiceCaptureDurationMs,
  normalizedAudioMimeType,
  validateVoiceCapture,
} from "./voice";
...
describe("voice capture validation", () => {
  ...
  it("rejects unsupported, empty, oversized, and overlong recordings", () => {
    expect(
      validateVoiceCapture({ mimeType: "audio/wav", byteSize: 12, durationMs: 1_000 }).ok,
    ).toBe(false);
    expect(
      validateVoiceCapture({ mimeType: "audio/webm", byteSize: 0, durationMs: 1_000 }).ok,
    ).toBe(false);
    expect(
      validateVoiceCapture({
        mimeType: "audio/webm",
        byteSize: maxVoiceCaptureBytes + 1,
        durationMs: 1_000,
      }).ok,
    ).toBe(false);
    expect(
      validateVoiceCapture({
        mimeType: "audio/webm",
        byteSize: 12,
        durationMs: maxVoiceCaptureDurationMs + 1,
      }).ok,
    ).toBe(false);
  });
  ...
  it("requires an idempotency identifier at the API boundary", () => {
    ...
    expect(
      createVoiceCaptureSchema.safeParse({ ...input, idempotencyKey: "not-an-id" }).success,
    ).toBe(false);
  });
});
