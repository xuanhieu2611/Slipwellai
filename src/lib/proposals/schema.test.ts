...
import { currentProposalEnvelopeSchema, destinationSelectionSchema, filedDateColumns, fileManuallySchema, parseProposalEnvelope, proposalActionSchema, proposalEnvelopeSchema } from "./schema";
...
describe("proposal schema", () => {
  ...
  it("accepts a review-first proposal", () => expect(proposalEnvelopeSchema.parse(v3).proposals).toHaveLength(1));
  it("rejects an unversioned response", () => expect(proposalEnvelopeSchema.safeParse({ ...v3, schemaVersion: "4" }).success).toBe(false));
  ...
  it("upgrades a stored version 1 proposal", () => {
    const upgraded = parseProposalEnvelope({ ...v1, proposals: [{ ...item, destinationName: "Rivera Studio", dueOn: "2026-08-12" }] });
  ...
  it("upgrades a version 2 date without inventing words for it", () => {
    const upgraded = parseProposalEnvelope({ ...v2, proposals: [{ ...item, dueOn: "2026-08-12", dueTime: "09:00" }] });
    expect(upgraded?.proposals[0]).toMatchObject({ date: "2026-08-12", time: "09:00", dateKind: "due" });
  ...
  it("keeps the capture's own date words with the date", () => {
    const parsed = currentProposalEnvelopeSchema.parse({
      ...v3,
      proposals: [{ ...item, datePhrase: "next Friday", date: "2026-08-14", dateKind: "scheduled", recurrence: { rule: "weekly", phrase: "every Friday" } }],
    });
    expect(parsed.proposals[0]).toMatchObject({ datePhrase: "next Friday", dateKind: "scheduled", recurrence: { rule: "weekly", phrase: "every Friday" } });
  });
});
...
describe("filed date input", () => {
  ...
  it("rejects a repeat with no first date, which the task table would reject too", () => {
    expect(fileManuallySchema.safeParse({ ...filed, recurrenceRule: "weekly" }).success).toBe(false);
    expect(fileManuallySchema.safeParse({ ...filed, recurrenceRule: "weekly", date: "2026-08-14" }).success).toBe(true);
    expect(proposalActionSchema.safeParse({ action: "accept", edited: { ...filed, recurrenceRule: "weekly" } }).success).toBe(false);
  });
...
describe("filedDateColumns", () => {
  it("separates a deadline from a start date", () => {
    expect(filedDateColumns({ dateKind: "due", date: "2026-08-14", time: "09:00" })).toMatchObject({ due_on: "2026-08-14", scheduled_for: null, due_time: "09:00" });
    expect(filedDateColumns({ dateKind: "scheduled", date: "2026-08-14" })).toMatchObject({ due_on: null, scheduled_for: "2026-08-14" });
  });

  it("anchors a repeat on its own date", () => {
    expect(filedDateColumns({ dateKind: "due", date: "2026-08-14", recurrenceRule: "weekly" })).toMatchObject({
      due_on: "2026-08-14",
  ...
  it("drops a repeat that lost its date", () => {
    expect(filedDateColumns({ recurrenceRule: "weekly" })).toMatchObject({ recurrence_rule: null, recurrence_anchor: null, scheduled_for: null });
  });

  it("writes nothing when no date was chosen", () => {
    expect(filedDateColumns({})).toEqual({ due_on: null, scheduled_for: null, due_time: null, recurrence_rule: null, recurrence_anchor: null });
  });
});
