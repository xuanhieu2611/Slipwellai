...
describe("OpenRouterProposalProvider", () => {
  it("requires ZDR and rejects malformed provider output without retaining it", async () => {
    ...
    await expect(new OpenRouterProposalProvider().propose({ captureId, originalText: "private text" })).rejects.toMatchObject({
  ...
  it("categorizes provider failures without exposing response bodies", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response("private echo", { status: 503 })));
    process.env.OPENROUTER_API_KEY = "test-key";
    process.env.OPENROUTER_MODEL = "test-model";
