...
export function normalizedAudioMimeType(value: string): AllowedAudioMimeType | null {
  ...
  return allowedAudioMimeTypes.includes(mimeType as AllowedAudioMimeType)
    ? (mimeType as AllowedAudioMimeType)
    : null;
}

export function validateVoiceCapture({
  mimeType,
  byteSize,
  durationMs,
}: {
  ...
  if (!normalizedMimeType)
    return { ok: false as const, error: "This browser recorded an unsupported audio format." };
  if (!Number.isFinite(byteSize) || byteSize < 1 || byteSize > maxVoiceCaptureBytes)
    return { ok: false as const, error: "Voice captures must be smaller than 25 MB." };
  if (!Number.isFinite(durationMs) || durationMs < 1 || durationMs > maxVoiceCaptureDurationMs)
    return { ok: false as const, error: "Voice captures can be up to five minutes long." };
  return { ok: true as const, mimeType: normalizedMimeType };
}

export const createVoiceCaptureSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (!normalizedAudioMimeType(value.mimeType))
      context.addIssue({
        code: "custom",
        path: ["mimeType"],
