...
describe.skipIf(!SUPABASE_URL || !SUPABASE_KEY)(
  "workspace integration (hosted pilot project)",
  () => {
    ...
    afterAll(async () => {
      if (createdSignalIds.length)
        await userA.from("slipping_signals").delete().in("id", createdSignalIds);
      ...
      if (createdRoutineIds.length)
        await userA.from("routines").delete().in("id", createdRoutineIds);
      ...
      await userA
        .from("user_preferences")
        .upsert({ owner_id: ownerAId, timezone: originalTimezone }, { onConflict: "owner_id" });
    });
    ...
    it("converges a retried create_task double-submit on one row instead of a duplicate", async () => {
      ...
      const first = await userA
        .from("tasks")
        .insert({ title, idempotency_key: idempotencyKey })
        .select("id")
        .single();
    ...
    it("blocks a duplicate generated recurrence occurrence at the recurrence_root_id + recurrence_anchor constraint", async () => {
      const root = await userA
        .from("tasks")
        .insert({
          title: "[integration-test] recurrence root",
          recurrence_rule: "daily",
          recurrence_anchor: "2026-09-01",
        })
        .select("id")
        .single();
      ...
      const occurrence = await userA
        .from("tasks")
        .insert({
          title: "[integration-test] recurrence occurrence",
          recurrence_rule: "daily",
          recurrence_root_id: root.data!.id,
          recurrence_anchor: "2026-09-02",
        })
        .select("id")
        .single();
      ...
      const retry = await userA
        .from("tasks")
        .insert({
          title: "[integration-test] recurrence occurrence retry",
          recurrence_rule: "daily",
          recurrence_root_id: root.data!.id,
          recurrence_anchor: "2026-09-02",
        })
    ...
    it("blocks a duplicate open Slipping signal at the (owner_id, entity_type, entity_id) partial unique index", async () => {
      const task = await userA
        .from("tasks")
        .insert({ title: "[integration-test] slipping dedup target" })
        .select("id")
        .single();
      ...
      const first = await userA
        ...
        .insert({
          ...
          reason: "[integration-test] no meaningful attention",
          severity: "attention",
        })
        .select("id")
        .single();
      ...
      const second = await userA.from("slipping_signals").insert({
        entity_type: "task",
        entity_id: task.data!.id,
        reason: "[integration-test] no meaningful attention",
        severity: "attention",
    ...
    it("accepts the cadence_changed outcome on a slipping_signals row (proves the enum value is live, not just Zod-valid)", async () => {
      const task = await userA
        .from("tasks")
        .insert({ title: "[integration-test] cadence_changed enum target" })
        .select("id")
        .single();
      expect(task.error).toBeNull();
      createdTaskIds.push(task.data!.id);

      const signal = await userA
        .from("slipping_signals")
        .insert({
          entity_type: "task",
          entity_id: task.data!.id,
          reason: "[integration-test] cadence_changed enum check",
          severity: "attention",
        })
        .select("id")
        .single();
      ...
      const updated = await userA
        .from("slipping_signals")
        .update({ outcome: "cadence_changed" })
        .eq("id", signal.data!.id)
        .select("outcome")
        .single();
    ...
    it("does not create an activity_events row for a cosmetic task edit that bypasses update_task", async () => {
      const task = await userA
        .from("tasks")
        .insert({ title: "[integration-test] cosmetic edit target" })
        .select("id")
        .single();
      expect(task.error).toBeNull();
      ...
      const update = await userA
        .from("tasks")
        .update({ title: "[integration-test] cosmetic edit target renamed" })
        .eq("id", taskId);
    ...
    it("resolves a task due on the correct local day near a UTC midnight boundary for a real confirmed timezone", async () => {
      await userA
        .from("user_preferences")
        .upsert({ owner_id: ownerAId, timezone: "America/Vancouver" }, { onConflict: "owner_id" });
      const { data: fetchedPrefs } = await userA
        .from("user_preferences")
        .select("timezone")
        .eq("owner_id", ownerAId)
        .single();
      ...
      expect(localDate(fetchedPrefs!.timezone, new Date("2026-08-06T06:59:00Z"))).toBe(
        "2026-08-05",
      );
      expect(localDate(fetchedPrefs!.timezone, new Date("2026-08-06T07:01:00Z"))).toBe(
        "2026-08-06",
      );

      const created = await userA
        .from("tasks")
        .insert({ title: "[integration-test] timezone boundary", due_on: "2026-08-06" })
        .select("id, due_on, scheduled_for, deferred_until")
        .single();
      expect(created.error).toBeNull();
    ...
    it("converges a retried create_project double-submit on one row instead of a duplicate", async () => {
      ...
      const first = await userA
        .from("projects")
        .insert({ name, idempotency_key: idempotencyKey })
        .select("id")
        .single();
      expect(first.error).toBeNull();
    ...
    it("mirrors archive_domain's blocking counts: an open task keeps the guard condition true, and it clears once the task is resolved", async () => {
      const domain = await userA
        .from("domains")
        .insert({ name: "[integration-test] domain with open task" })
        .select("id")
        .single();
      ...
      const task = await userA
        .from("tasks")
        .insert({ title: "[integration-test] domain-blocking task", domain_id: domainId })
        .select("id")
        .single();
      expect(task.error).toBeNull();
      const taskId = task.data!.id;
      ...
      const blockedCount = await userA
        .from("tasks")
      ...
      const clearedCount = await userA
        .from("tasks")
        .select("id", { count: "exact", head: true })
        .eq("domain_id", domainId)
        .eq("status", "open")
    ...
    it("mirrors delete_person/restore_person and delete_note/restore_note: toggling archived_at is the whole soft-delete/recovery mechanism", async () => {
      const person = await userA
        .from("people")
        .insert({ name: "[integration-test] person delete/restore" })
      ...
      const deletedPerson = await userA
        .from("people")
        .update({ archived_at: new Date().toISOString() })
        .eq("id", personId)
        .select("archived_at")
      ...
      const restoredPerson = await userA
        .from("people")
      ...
      const note = await userA
        .from("notes")
        .insert({ title: "[integration-test] note delete/restore" })
        .select("id, archived_at")
        .single();
      ...
      const restoredNote = await userA
        .from("notes")
        .update({ archived_at: null })
        .eq("id", noteId)
        .select("archived_at")
    ...
    it("keeps a second account from reading, updating, or deleting another user's routine, and blocks a duplicate completion at the (routine_id, local_date) constraint", async () => {
      const routine = await userA
        .from("routines")
        .insert({ name: "[integration-test] owned by user A routine" })
        .select("id")
        .single();
      ...
      const first = await userA
        ...
        .insert({ routine_id: routineId, local_date: "2026-09-01", outcome: "completed" })
        .select("id")
        .single();
      expect(first.error).toBeNull();
      const second = await userA
        .from("routine_completions")
        .insert({ routine_id: routineId, local_date: "2026-09-01", outcome: "skipped" });
      expect(second.error?.code).toBe("23505");

      // A second account cannot read this account's completion history for the routine either.
      const completionRead = await userB
        .from("routine_completions")
        .select("id")
    ...
    it("keeps a second account from reading, updating, or deleting another user's project milestone", async () => {
      const project = await userA
        .from("projects")
        .insert({ name: "[integration-test] milestone owner project" })
        .select("id")
        .single();
      ...
      const milestone = await userA
        .from("project_milestones")
        .insert({
          project_id: projectId,
          title: "[integration-test] owned by user A milestone",
          position: 1,
        })
        .select("id")
        .single();
    ...
    it("keeps a second account from reading, updating, or deleting another user's person interaction", async () => {
      const person = await userA
        .from("people")
        .insert({ name: "[integration-test] interaction owner person" })
        .select("id")
        .single();
      expect(person.error).toBeNull();
      const personId = person.data!.id;
      ...
      const interaction = await userA
        .from("person_interactions")
        .insert({ person_id: personId, summary: "[integration-test] owned by user A interaction" })
        .select("id")
        .single();
    ...
    it("keeps a second account from reading, updating, or deleting another user's capture", async () => {
      ...
      const created = await userA
        .from("captures")
        .insert({ original_text: originalText, idempotency_key: crypto.randomUUID() })
        .select("id")
        .single();
      expect(created.error).toBeNull();
    ...
    it("keeps a second account from reading, updating, or deleting another user's proposal, and confirms the proposal's private capture text stays unreadable too", async () => {
      const capture = await userA
        .from("captures")
        .insert({
          original_text: "[integration-test] proposal owner capture",
      ...
      const proposal = await userA
        .from("proposals")
        .insert({ capture_id: captureId, proposal_json: { schemaVersion: 3, items: [] } })
        .select("id")
        .single();
      ...
      const joinAttempt = await userB
        .from("proposals")
        .select("id, captures(original_text)")
    ...
    it("keeps a second account from reading or updating another user's preferences row", async () => {
      ...
      const update = await userB
        .from("user_preferences")
        .update({ timezone: "hijacked/timezone" })
        .eq("owner_id", ownerAId)
        .select();
      expect(update.error).toBeNull();
      expect(update.data).toEqual([]);
    ...
    it("blocks a hard delete of an applied checklist template item at the on-delete-restrict constraint, and confirms the soft-delete (archived_at) path leaves applied checklists intact", async () => {
      const template = await userA
        .from("project_checklist_templates")
        .insert({ name: "[integration-test] template" })
        .select("id, version")
        .single();
      ...
      const item = await userA
        .from("project_checklist_template_items")
        .insert({ template_id: templateId, title: "Draft the report", position: 1 })
        .select("id")
        .single();
      ...
      const project = await userA
        .from("projects")
        .insert({ name: "[integration-test] project with checklist" })
        .select("id")
        .single();
      expect(project.error).toBeNull();
      const projectId = project.data!.id;
      ...
      const instance = await userA
        .from("project_checklist_instances")
        .insert({
          project_id: projectId,
      ...
      const checklistItem = await userA
        .from("project_checklist_items")
        .insert({
          instance_id: instance.data!.id,
          source_template_item_id: itemId,
      ...
      const confirmChecklistItem = await userA
        .from("project_checklist_items")
        .select("title")
        .eq("id", checklistItem.data!.id)
        .single();
    ...
    it("mirrors the complete_project guard at the database layer: an open task and an open checklist item each block completion until resolved", async () => {
      const project = await userA
        .from("projects")
        .insert({ name: "[integration-test] guarded completion project" })
        .select("id")
        .single();
      expect(project.error).toBeNull();
      const projectId = project.data!.id;
      ...
      const task = await userA
        .from("tasks")
        .insert({ title: "[integration-test] guard-blocking task", project_id: projectId })
        .select("id, status")
        .single();
      expect(task.error).toBeNull();
      const taskId = task.data!.id;
      ...
      const openTaskGuard = await userA
        .from("tasks")
        .select("id", { count: "exact", head: true })
      ...
      const template = await userA
        .from("project_checklist_templates")
        .insert({ name: "[integration-test] guard template" })
        .select("id, version")
        .single();
      expect(template.error).toBeNull();
      const templateId = template.data!.id;
      ...
      const templateItem = await userA
        .from("project_checklist_template_items")
        .insert({ template_id: templateId, title: "Sign off", position: 1 })
        .select("id")
        .single();
      ...
      const checklistItem = await userA
        ...
        .insert({
          ...
          source_template_item_id: templateItem.data!.id,
          title: "Sign off",
          position: 1,
        })
        .select("id, status")
      ...
      const completedTask = await userA
        .from("tasks")
      ...
      const openTaskGuardAfter = await userA
        .from("tasks")
        .select("id", { count: "exact", head: true })
        .eq("project_id", projectId)
        .eq("status", "open");
      ...
      const openChecklistGuardAfter = await userA
        .from("project_checklist_items")
        .select("id", { count: "exact", head: true })
        .in("instance_id", [instanceId])
        .eq("status", "open");
      ...
      const completedProject = await userA
        .from("projects")
    ...
    it("converges a retried retainer cycle generation double-submit on one cycle and one set of items, then carries a still-open item forward into the next cycle", async () => {
      const retainer = await userA
        .from("retainers")
        .insert({ name: "[integration-test] retainer", cycle_day: 1 })
        .select("id")
        .single();
      ...
      const templateItem = await userA
        ...
        .insert({ retainer_id: retainerId, title: "Monthly report", expected_day: 1 })
        .select("id, version")
        .single();
      expect(templateItem.error).toBeNull();
      ...
      const septemberItems = [
        {
          ...
          sourceTemplateVersion: templateItem.data!.version,
          title: "Monthly report",
          expectedOn: expectedDate("2026-09", 1),
        },
      ];
      const idempotencyKey = crypto.randomUUID();

      const first = await userA.rpc("generate_retainer_cycle", {
        p_retainer_id: retainerId,
        p_cycle_start: septemberBounds.start,
        p_cycle_end: septemberBounds.end,
        p_idempotency_key: idempotencyKey,
        p_new_items: septemberItems,
        p_carry_forward_items: [],
      });
      expect(first.error).toBeNull();

      const retried = await userA.rpc("generate_retainer_cycle", {
        p_retainer_id: retainerId,
        p_cycle_start: septemberBounds.start,
        p_cycle_end: septemberBounds.end,
        p_idempotency_key: idempotencyKey,
      ...
      const septemberCycles = await userA
        .from("retainer_cycles")
        .select("id, generation_status")
        .eq("retainer_id", retainerId)
        .eq("cycle_start", septemberBounds.start);
      ...
      const septemberCycleItems = await userA
        .from("retainer_cycle_items")
        .select("id, status")
        .eq("cycle_id", septemberCycleId);
      ...
      const octoberNewItems = [
        {
          sourceTemplateItemId: templateItemId,
          sourceTemplateVersion: templateItem.data!.version,
          title: "Monthly report",
          expectedOn: expectedDate("2026-10", 1),
        },
      ];
      const octoberCarryForward = [
        {
          sourceTemplateItemId: templateItemId,
          sourceTemplateVersion: templateItem.data!.version,
          carriedFromItemId: septemberItemId,
          title: "Monthly report (carried forward)",
          expectedOn: octoberBounds.start,
        },
      ];
      const october = await userA.rpc("generate_retainer_cycle", {
        p_retainer_id: retainerId,
        p_cycle_start: octoberBounds.start,
        p_cycle_end: octoberBounds.end,
        p_idempotency_key: crypto.randomUUID(),
        p_new_items: octoberNewItems,
        p_carry_forward_items: octoberCarryForward,
      });
      expect(october.error).toBeNull();

      const octoberCycles = await userA
        .from("retainer_cycles")
        .select("id")
        .eq("retainer_id", retainerId)
        .eq("cycle_start", octoberBounds.start)
        .single();
      const octoberCycleItems = await userA
        .from("retainer_cycle_items")
        .select("carried_from_item_id, title")
        .eq("cycle_id", octoberCycles.data!.id);
      expect(octoberCycleItems.data).toHaveLength(2);
      expect(octoberCycleItems.data).toContainEqual({
        carried_from_item_id: septemberItemId,
        title: "Monthly report (carried forward)",
      });

      const confirmSeptemberItem = await userA
        .from("retainer_cycle_items")
        .select("status")
        .eq("id", septemberItemId)
        .single();
    ...
    it("resumes a partially populated cycle instead of duplicating or erroring on retry", async () => {
      const retainer = await userA
        .from("retainers")
        .insert({ name: "[integration-test] resumable retainer", cycle_day: 1 })
        .select("id")
        .single();
      expect(retainer.error).toBeNull();
      const retainerId = retainer.data!.id;
      ...
      const partialCycle = await userA
        .from("retainer_cycles")
        .insert({
      ...
      const partialItem = await userA
        .from("retainer_cycle_items")
        .insert({
          cycle_id: partialCycle.data!.id,
          source_template_item_id: itemA.id,
          source_template_version: itemA.version,
          title: itemA.title,
          expected_on: expectedDate("2026-11", 1),
        })
        .select("id")
        .single();
      ...
      const newItems = [
        {
          sourceTemplateItemId: itemA.id,
          sourceTemplateVersion: itemA.version,
          title: itemA.title,
          expectedOn: expectedDate("2026-11", 1),
        },
        {
          sourceTemplateItemId: itemB.id,
          sourceTemplateVersion: itemB.version,
          title: itemB.title,
          expectedOn: expectedDate("2026-11", 15),
        },
      ];
      const resumed = await userA.rpc("generate_retainer_cycle", {
        p_retainer_id: retainerId,
      ...
      const cycle = await userA
        .from("retainer_cycles")
        .select("id, generation_status")
        .eq("retainer_id", retainerId)
        .eq("cycle_start", bounds.start)
        .single();
      expect(cycle.data?.generation_status).toBe("complete");
      const cycleItems = await userA
        .from("retainer_cycle_items")
      ...
      expect(cycleItems.data!.find((row) => row.source_template_item_id === itemA.id)?.id).toBe(
        partialItem.data!.id,
      );
    });
    ...
    it("blocks a hard delete of a retainer deliverable template item that has already produced cycle work, and confirms the soft-delete (archived_at) path leaves that work intact", async () => {
      const retainer = await userA
        .from("retainers")
        .insert({ name: "[integration-test] template restrict retainer", cycle_day: 1 })
        .select("id")
        .single();
      expect(retainer.error).toBeNull();
      const retainerId = retainer.data!.id;
      ...
      const cycle = await userA
        .from("retainer_cycles")
        .insert({ retainer_id: retainerId, cycle_start: bounds.start, cycle_end: bounds.end })
        .select("id")
        .single();
      expect(cycle.error).toBeNull();
      const cycleItem = await userA
        .from("retainer_cycle_items")
        .insert({
          cycle_id: cycle.data!.id,
          source_template_item_id: templateItemId,
          source_template_version: templateItem.data!.version,
          title: "Monthly report",
          expected_on: expectedDate("2026-12", 1),
        })
        .select("id, title")
        .single();
      ...
      const confirmCycleItem = await userA
        .from("retainer_cycle_items")
        .select("title")
        .eq("id", cycleItem.data!.id)
        .single();
    ...
    it("keeps a carried-forward-from chain linked correctly three cycles deep instead of resetting or breaking after one hop", async () => {
      const retainer = await userA
        .from("retainers")
        .insert({ name: "[integration-test] chain retainer", cycle_day: 1 })
        .select("id")
        .single();
      expect(retainer.error).toBeNull();
      const retainerId = retainer.data!.id;
      ...
      const templateItem = await userA
        .from("retainer_deliverable_templates")
        .insert({ retainer_id: retainerId, title: "Quarterly audit", expected_day: 1 })
        .select("id, version")
        .single();
      const templateItemId = templateItem.data!.id;
      ...
      async function generate(month: string, priorItemId: string | null) {
          ...
          : [
              {
                ...
                expectedOn: expectedDate(month, 1),
              },
            ];
        const carryForwardItems = priorItemId
          ? [
              {
                sourceTemplateItemId: templateItemId,
                sourceTemplateVersion: templateItem.data!.version,
                carriedFromItemId: priorItemId,
                title: "Quarterly audit (carried forward)",
                expectedOn: bounds.start,
              },
            ]
          : [];
        const result = await userA.rpc("generate_retainer_cycle", {
          p_retainer_id: retainerId,
          p_cycle_start: bounds.start,
          p_cycle_end: bounds.end,
          p_idempotency_key: crypto.randomUUID(),
        ...
        const cycleItems = await userA
          ...
          .eq("cycle_id", result.data![0].out_cycle_id);
      ...
      const walkedBackToFebruary = await userA
        .from("retainer_cycle_items")
        .select("carried_from_item_id")
        .eq("id", march.carried_from_item_id)
        .single();
    ...
    it("excludes an item explicitly left in its prior cycle from the carry-forward candidate set, while a sibling open item remains eligible", async () => {
      const retainer = await userA
        .from("retainers")
        .insert({ name: "[integration-test] leave-in-prior retainer", cycle_day: 1 })
        .select("id")
        .single();
      expect(retainer.error).toBeNull();
      const retainerId = retainer.data!.id;
      ...
      const leftBehindItem = await userA
        .from("retainer_cycle_items")
        .insert({
          cycle_id: cycleId,
          source_template_item_id: leftBehindTemplate.id,
          source_template_version: leftBehindTemplate.version,
          title: leftBehindTemplate.title,
          expected_on: bounds.start,
        })
        .select("id")
        .single();
      const eligibleItem = await userA
        .from("retainer_cycle_items")
        .insert({
          cycle_id: cycleId,
          source_template_item_id: eligibleTemplate.id,
          source_template_version: eligibleTemplate.version,
          title: eligibleTemplate.title,
          expected_on: bounds.start,
        })
        .select("id")
        .single();
      ...
      const excluded = await userA
        .from("retainer_cycle_items")
        .update({ excluded_from_carry_forward: true })
        .eq("id", leftBehindItem.data!.id)
        .select("status, excluded_from_carry_forward")
        .single();
      ...
      const closed = await userA
        .from("retainer_cycle_items")
        .update({ status: "closed" })
        .eq("id", eligibleItem.data!.id)
        .select("status")
        .single();
    ...
    it("transitions a retainer through active to paused and back to active", async () => {
      const retainer = await userA
        .from("retainers")
        .insert({ name: "[integration-test] pause/resume retainer", cycle_day: 1 })
        .select("id, status")
        .single();
      expect(retainer.error).toBeNull();
      const retainerId = retainer.data!.id;
    ...
    it("closes every still-open cycle item across every cycle when a retainer ends with the close_all resolution", async () => {
      const retainer = await userA
        .from("retainers")
        .insert({ name: "[integration-test] end-retainer close-all", cycle_day: 1 })
        .select("id")
        .single();
      expect(retainer.error).toBeNull();
      const retainerId = retainer.data!.id;
      ...
      const juneCycle = await userA
        .from("retainer_cycles")
        .insert({
          retainer_id: retainerId,
          cycle_start: juneBounds.start,
          cycle_end: juneBounds.end,
        })
        .select("id")
        .single();
      const julyCycle = await userA
        .from("retainer_cycles")
        .insert({
          retainer_id: retainerId,
          cycle_start: julyBounds.start,
          cycle_end: julyBounds.end,
        })
        .select("id")
        .single();
      ...
      const template = await userA
        .from("retainer_deliverable_templates")
        .insert({ retainer_id: retainerId, title: "Recurring deliverable", expected_day: 1 })
        .select("id, version")
        .single();
      const juneItem = await userA
        .from("retainer_cycle_items")
        .insert({
          cycle_id: juneCycle.data!.id,
          source_template_item_id: template.data!.id,
          source_template_version: template.data!.version,
          title: "Recurring deliverable",
          expected_on: juneBounds.start,
        })
        .select("id")
        .single();
      const julyItem = await userA
        .from("retainer_cycle_items")
        .insert({
          cycle_id: julyCycle.data!.id,
          source_template_item_id: template.data!.id,
          source_template_version: template.data!.version,
          title: "Recurring deliverable",
          expected_on: julyBounds.start,
        })
        .select("id")
        .single();
      ...
      const confirmJune = await userA
        .from("retainer_cycle_items")
        .select("status")
        .eq("id", juneItem.data!.id)
        .single();
      const confirmJuly = await userA
        .from("retainer_cycle_items")
        .select("status")
        .eq("id", julyItem.data!.id)
        .single();
      expect(confirmJune.data?.status).toBe("closed");
      expect(confirmJuly.data?.status).toBe("closed");

      const ended = await userA
        .from("retainers")
        .update({ status: "ended" })
        .eq("id", retainerId)
        .select("status")
    ...
    it("converges a retried create_retainer double-submit on one row instead of a duplicate, and links a task to it via retainer_id", async () => {
      ...
      const first = await userA
        .from("retainers")
        .insert({ name, cycle_day: 1, idempotency_key: idempotencyKey })
        .select("id")
        .single();
      expect(first.error).toBeNull();
      ...
      const task = await userA
        .from("tasks")
        .insert({ title: "[integration-test] retainer-linked task", retainer_id: retainerId })
        .select("id, retainer_id")
        .single();
      expect(task.error).toBeNull();
      createdTaskIds.push(task.data!.id);
