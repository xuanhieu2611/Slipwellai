...
describe("filedDestinationLabel", () => {
  ...
  it("names every destination the record actually carries", () => {
    expect(filedDestinationLabel({ domain_id: "domain-1", project_id: "project-1", person_id: "person-1" }, catalog)).toBe(
      "Rivera brand refresh · Dana Rivera · Client work",
    );
  });

  it("says nothing rather than implying a destination the record does not have", () => {
    expect(filedDestinationLabel({ domain_id: null, project_id: null, person_id: null }, catalog)).toBeNull();
  });
  ...
  it("omits a destination that is no longer in the catalog", () => {
    expect(filedDestinationLabel({ domain_id: "domain-1", project_id: "archived-project", person_id: null }, catalog)).toBe("Client work");
  });
});
