...
export class TranscriptionError extends Error {
  constructor(public readonly code: "transcription_not_configured" | "transcription_provider_error" | "transcription_invalid_response", message: string) {
    super(message);
  }
}
...
export async function transcribeAudio({ audio, mimeType }: { audio: Blob; mimeType: string }) {
  ...
  if (!response.ok) throw new TranscriptionError("transcription_provider_error", "The transcription provider could not process this recording.");
  const payload: unknown = await response.json();
  if (!payload || typeof payload !== "object" || !("text" in payload) || typeof payload.text !== "string" || !payload.text.trim()) {
    throw new TranscriptionError("transcription_invalid_response", "The transcription provider returned an invalid transcript.");
  }
