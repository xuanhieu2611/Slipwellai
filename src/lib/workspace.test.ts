...
describe("working-prototype workspace commands", () => {
  ...
  it("keeps due, scheduled, and deferred semantics distinct in the UI", () => {
    expect(
      taskDateLabel({ due_on: "2026-08-04", scheduled_for: "2026-08-03", deferred_until: null }),
    ).toBe("Due 2026-08-04");
    expect(taskDateLabel({ due_on: null, scheduled_for: "2026-08-03", deferred_until: null })).toBe(
      "Scheduled 2026-08-03",
    );
    expect(
      taskDateLabel({ due_on: "2026-08-04", scheduled_for: null, deferred_until: "2026-08-06" }),
    ).toBe("Deferred until 2026-08-06");
  });
  ...
  it("accepts a template application only with valid project and template identities", () => {
    expect(
      workspaceCommandSchema.safeParse({
        ...
        templateId: "6f1d9b6d-7a94-4de2-bf85-14da8b7c6b98",
        projectId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
      }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({
  ...
  it("accepts the null-for-blank payloads the creation forms actually post", () => {
    const posted = [
      ...
      },
      {
        ...
        name: "Launch the September report",
        description: null,
        domainId: null,
        personId: null,
        startOn: null,
        targetOn: null,
        idempotencyKey: "6f1d9b6d-7a94-4de2-bf85-14da8b7c6b98",
      },
      {
        action: "create_task",
        title: "Send the report",
      ...
      { action: "create_checklist_template", name: "Monthly client report", description: null },
      {
        ...
        personId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
        summary: "Discussed the report",
        followUpTitle: null,
      },
    ];
    for (const command of posted)
      expect(workspaceCommandSchema.safeParse(command), command.action).toMatchObject({
        success: true,
      });
  });

  it("normalizes blank optional text to null rather than an empty string", () => {
    const parsed = workspaceCommandSchema.parse({
      action: "create_domain",
      name: "Client work",
      description: null,
      color: "#2348c8",
    ...
    ).toMatchObject({ description: "Trim me" });
  });

  it("still rejects required text that is missing or blank", () => {
    expect(workspaceCommandSchema.safeParse({ action: "create_domain", name: null }).success).toBe(
      false,
    );
    expect(workspaceCommandSchema.safeParse({ action: "create_task", title: "   " }).success).toBe(
      false,
    );
  });
  ...
  it("requires a private person identity and interaction summary", () => {
    expect(
      workspaceCommandSchema.safeParse({
        action: "create_person_interaction",
        personId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
        summary: "Discussed next month’s report",
        followUpTitle: "Send recap",
      }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({
  ...
  it("accepts a task update with the null-for-blank shape the edit form posts, and rejects a missing title", () => {
    const posted = {
      action: "update_task",
      taskId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
      title: "Send the revised report",
      details: null,
      dueOn: null,
      scheduledFor: null,
      priority: "2",
    ...
    expect(workspaceCommandSchema.safeParse({ ...posted, taskId: "not-an-id" }).success).toBe(
      false,
    );
  });
  ...
  it("lets a task update repoint domain, project, and person, mirroring create_task's relation fields", () => {
    const parsed = workspaceCommandSchema.parse({
      ...
      projectId: "6f1d9b6d-7a94-4de2-bf85-14da8b7c6b98",
      personId: "6f1d9b6d-7a94-4de2-bf85-14da8b7c6b98",
    });
  ...
  it("accepts update/delete/restore for people and notes with the null-for-blank shape the edit forms post, and rejects a missing name/title", () => {
    ...
    expect(
      workspaceCommandSchema.safeParse({ ...postedPerson, personId: "not-an-id" }).success,
    ).toBe(false);
    expect(workspaceCommandSchema.safeParse({ action: "delete_person", personId }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ action: "restore_person", personId }).success).toBe(
      true,
    );
    ...
    expect(workspaceCommandSchema.safeParse({ ...postedNote, noteId: "not-an-id" }).success).toBe(
      false,
    );
  ...
  it("prioritizes a deferred date over due/scheduled dates when deciding what belongs on a given day, matching taskDateLabel's own priority", () => {
    expect(
      isTaskOnDay(
        { due_on: "2026-08-06", scheduled_for: null, deferred_until: "2026-08-10" },
        "2026-08-06",
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      isTaskOnDay(
        { due_on: "2026-08-06", scheduled_for: null, deferred_until: null },
        "2026-08-06",
      ),
    ).toBe(true);
    expect(
      isTaskOnDay({ due_on: null, scheduled_for: null, deferred_until: null }, "2026-08-06"),
    ).toBe(false);
  });
  ...
  it("uses the same planning-date priority for calendar placement and labels", () => {
    ...
    expect(
      taskPlanningDate({ due_on: "2026-08-06", scheduled_for: "2026-08-05", deferred_until: null }),
    ).toBe("2026-08-06");
    expect(
      taskPlanningDate({ due_on: null, scheduled_for: "2026-08-05", deferred_until: null }),
    ).toBe("2026-08-05");
    expect(
      taskPlanningDate({ due_on: null, scheduled_for: null, deferred_until: null }),
    ).toBeNull();
  });
  ...
  it("accepts cancel, delete, and restore commands for a valid task id and rejects a bad one", () => {
    ...
    expect(
      workspaceCommandSchema.safeParse({ action: "cancel_task", taskId: "not-an-id" }).success,
    ).toBe(false);
  });
  ...
  it("requires a stable idempotency key to create a project, and accepts its person/start-date fields", () => {
    ...
    expect(workspaceCommandSchema.safeParse({ ...base, idempotencyKey: undefined }).success).toBe(
      false,
    );
    const parsed = workspaceCommandSchema.parse({
      ...base,
  ...
  it("accepts a project update with the null-for-blank shape the edit form posts, and rejects a missing name", () => {
    const posted = {
      ...
      projectId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
      name: "Launch the revised report",
      description: null,
      domainId: null,
      personId: null,
      startOn: null,
    ...
    expect(workspaceCommandSchema.safeParse({ ...posted, projectId: "not-an-id" }).success).toBe(
      false,
    );
  });
  ...
  it("accepts resume, cancel, delete, restore, and milestone-delete commands for a valid id and rejects a bad one", () => {
    ...
    expect(workspaceCommandSchema.safeParse({ action: "resume_project", projectId }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ action: "cancel_project", projectId }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ action: "delete_project", projectId }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ action: "restore_project", projectId }).success).toBe(
      true,
    );
    expect(
      workspaceCommandSchema.safeParse({ action: "delete_milestone", milestoneId: projectId })
        .success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ action: "cancel_project", projectId: "not-an-id" })
        .success,
    ).toBe(false);
  });
  ...
  it("requires a stable idempotency key to create a task, and defaults tags to an empty array", () => {
    ...
    expect(workspaceCommandSchema.parse({ ...base, tags: ["client", "urgent"] })).toMatchObject({
      tags: ["client", "urgent"],
    });
  });
  ...
  it("accepts pronouns and tags on person creation, mirroring create_task's tags field, and defaults both to blank/empty", () => {
    ...
    expect(workspaceCommandSchema.parse({ ...base, pronouns: "she/her", tags: ["client", "rivera-studio"] })).toMatchObject({ pronouns: "she/her", tags: ["client", "rivera-studio"] });
    expect(workspaceCommandSchema.parse({ ...base, pronouns: null })).toMatchObject({ pronouns: null });
    expect(workspaceCommandSchema.safeParse({ ...base, pronouns: "x".repeat(61) }).success).toBe(false);
    expect(workspaceCommandSchema.safeParse({ ...base, tags: Array.from({ length: 21 }, (_, index) => `tag-${index}`) }).success).toBe(false);
  });
  ...
  it("lets a person update carry pronouns and tags, mirroring create_person's fields, and defaults both to blank/empty", () => {
    ...
    expect(workspaceCommandSchema.parse(base)).toMatchObject({ pronouns: null, tags: [] });
    expect(workspaceCommandSchema.parse({ ...base, pronouns: "she/her", tags: ["client"] })).toMatchObject({ pronouns: "she/her", tags: ["client"] });
  });

  it("accepts tags on note creation and update, mirroring create_task's field, and defaults to an empty array", () => {
    const noteId = "6f1d9b6d-7a94-4de2-bf85-14da8b7c6b98";
    expect(workspaceCommandSchema.parse({ action: "create_note", title: "Call notes", body: null })).toMatchObject({ tags: [] });
    expect(workspaceCommandSchema.parse({ action: "create_note", title: "Call notes", body: null, tags: ["client", "ideas"] })).toMatchObject({ tags: ["client", "ideas"] });
    expect(workspaceCommandSchema.parse({ action: "update_note", noteId, title: "Call notes" })).toMatchObject({ tags: [] });
    expect(workspaceCommandSchema.parse({ action: "update_note", noteId, title: "Call notes", tags: ["client"] })).toMatchObject({ tags: ["client"] });
  });
  ...
  it("requires an interval and unit for a custom repeat, and rejects an interval outside 1-30", () => {
    ...
    expect(
      workspaceCommandSchema.safeParse({ ...base, recurrenceInterval: 3, recurrenceUnit: "days" })
        .success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ ...base, recurrenceInterval: 31, recurrenceUnit: "days" })
        .success,
    ).toBe(false);
  });
  ...
  it("accepts an archive_domain command only with a valid domain id", () => {
    ...
    expect(
      workspaceCommandSchema.safeParse({ action: "archive_domain", domainId: "not-an-id" }).success,
    ).toBe(false);
  });
  ...
  it("accepts an update_domain command with the null-for-blank shape the edit form posts", () => {
    ...
    expect(
      workspaceCommandSchema.parse({ ...posted, description: "  Ads retainer  " }),
    ).toMatchObject({ description: "Ads retainer" });
    expect(workspaceCommandSchema.safeParse({ ...posted, name: "   " }).success).toBe(false);
    expect(workspaceCommandSchema.safeParse({ ...posted, color: "blue" }).success).toBe(false);
    expect(workspaceCommandSchema.safeParse({ ...posted, domainId: "not-an-id" }).success).toBe(
      false,
    );
  });
  ...
  it("accepts a domain's default attention cadence within 1-365 and rejects values outside that range", () => {
    ...
    expect(workspaceCommandSchema.safeParse({ ...create, slippingCadenceDays: 1 }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ ...create, slippingCadenceDays: 365 }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ ...create, slippingCadenceDays: 0 }).success).toBe(
      false,
    );
    expect(workspaceCommandSchema.safeParse({ ...create, slippingCadenceDays: 366 }).success).toBe(
      false,
    );
    expect(
      workspaceCommandSchema.parse({ ...create, slippingCadenceDays: undefined }),
    ).toMatchObject({ slippingCadenceDays: undefined });
    ...
    expect(workspaceCommandSchema.safeParse({ ...update, slippingCadenceDays: 1 }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ ...update, slippingCadenceDays: 365 }).success).toBe(
      true,
    );
    expect(workspaceCommandSchema.safeParse({ ...update, slippingCadenceDays: 0 }).success).toBe(
      false,
    );
    expect(workspaceCommandSchema.safeParse({ ...update, slippingCadenceDays: 366 }).success).toBe(
      false,
    );
  });
  ...
  it("accepts checklist template item edit/delete and template delete for a valid id, defaulting applyToExisting to false", () => {
    ...
    const parsed = workspaceCommandSchema.parse({
      action: "update_checklist_template_item",
      itemId,
    ...
    ).toMatchObject({ applyToExisting: true });
    expect(
      workspaceCommandSchema.safeParse({
        action: "update_checklist_template_item",
        itemId,
    ...
    expect(
      workspaceCommandSchema.safeParse({ action: "delete_checklist_template_item", itemId })
        .success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ action: "delete_checklist_template", templateId }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({
  ...
  it("accepts a retainer template item delete for a valid id and rejects a bad one", () => {
    expect(
      workspaceCommandSchema.safeParse({
        action: "delete_retainer_template_item",
        itemId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
      }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({
  ...
  it("accepts close and leave-in-prior-cycle commands for a valid retainer cycle item id and rejects a bad one", () => {
    const itemId = "847a0e15-63ef-4a68-98f7-51fdbe09f29d";
    expect(
      workspaceCommandSchema.safeParse({ action: "close_retainer_cycle_item", itemId }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({
        action: "leave_retainer_cycle_item_in_prior_cycle",
        itemId,
      }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ action: "close_retainer_cycle_item", itemId: "not-an-id" })
        .success,
    ).toBe(false);
  });
  ...
  it("accepts pause and resume retainer commands for a valid id and rejects a bad one", () => {
    ...
    expect(workspaceCommandSchema.safeParse({ action: "pause_retainer", retainerId }).success).toBe(
      true,
    );
    expect(
      workspaceCommandSchema.safeParse({ action: "resume_retainer", retainerId }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ action: "pause_retainer", retainerId: "not-an-id" })
        .success,
    ).toBe(false);
  });
  ...
  it("requires an explicit open-item resolution to end a retainer, rather than defaulting one silently", () => {
    ...
    expect(
      workspaceCommandSchema.safeParse({
        action: "end_retainer",
        retainerId,
        openItemResolution: "close_all",
      }).success,
    ).toBe(true);
    expect(workspaceCommandSchema.safeParse({ action: "end_retainer", retainerId }).success).toBe(
      false,
    );
    expect(
      workspaceCommandSchema.safeParse({
        action: "end_retainer",
        retainerId,
        openItemResolution: "silently_decide",
      }).success,
    ).toBe(false);
  });
  ...
  it("requires a stable idempotency key to create a retainer, and accepts its client/domain links", () => {
    const base = {
      ...
      name: "Rivera Studio monthly retainer",
    ...
    const parsed = workspaceCommandSchema.parse({
      ...base,
      clientPersonId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
      domainId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
    });
    expect(parsed).toMatchObject({
      clientPersonId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
      domainId: "847a0e15-63ef-4a68-98f7-51fdbe09f29d",
  ...
  it("accepts delete and restore commands for a valid retainer id and rejects a bad one", () => {
    const retainerId = "847a0e15-63ef-4a68-98f7-51fdbe09f29d";
    expect(
      workspaceCommandSchema.safeParse({ action: "delete_retainer", retainerId }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ action: "restore_retainer", retainerId }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ action: "delete_retainer", retainerId: "not-an-id" })
        .success,
    ).toBe(false);
  });
  ...
  it("counts a routine's current streak, treating an unresolved today as still open but an unresolved earlier day as a break", () => {
    expect(
      routineCurrentStreak(
        [
          { local_date: "2026-08-06", outcome: "completed" },
          { local_date: "2026-08-07", outcome: "completed" },
          { local_date: "2026-08-08", outcome: "completed" },
        ],
        "2026-08-08",
      ),
    ).toBe(3);
    ...
    expect(
      routineCurrentStreak(
        [
          { local_date: "2026-08-05", outcome: "completed" },
          { local_date: "2026-08-07", outcome: "completed" },
        ],
        "2026-08-08",
      ),
    ...
    expect(
      routineCurrentStreak([{ local_date: "2026-08-07", outcome: "skipped" }], "2026-08-08"),
    ).toBe(0);
  ...
  it("builds a 53-week, Monday-first heatmap grid ending on today's week, with a month label only where the month changes", () => {
    const weeks = routineHeatmapWeeks(
      [
        { local_date: "2026-08-06", outcome: "completed" },
        { local_date: "2026-08-07", outcome: "skipped" },
      ],
      "2026-08-08",
      53,
    );
  ...
  it("accepts complete and reopen commands for a valid retainer cycle item id and rejects a bad one", () => {
    const itemId = "847a0e15-63ef-4a68-98f7-51fdbe09f29d";
    expect(
      workspaceCommandSchema.safeParse({ action: "complete_retainer_cycle_item", itemId }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({ action: "reopen_retainer_cycle_item", itemId }).success,
    ).toBe(true);
    expect(
      workspaceCommandSchema.safeParse({
        action: "complete_retainer_cycle_item",
        itemId: "not-an-id",
      }).success,
    ).toBe(false);
  });
