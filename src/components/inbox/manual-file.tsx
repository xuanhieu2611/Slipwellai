...
export function ManualFile({
  ...
  const [draft, setDraft] = useState({
    recordType: "task" as "task" | "note",
    title: capture.original_text.slice(0, 120),
  });
  ...
  return (
    <form className="review-fields w-full" onSubmit={submit}>
      ...
      <label className="field-label">
        <span>File it as</span>
        <SelectField
          onChange={(event) =>
            setDraft({ ...draft, recordType: event.target.value as "task" | "note" })
          }
      ...
      <div className="form-span flex flex-wrap gap-2">
        <Button className="button-primary" disabled={busy || !draft.title.trim()} type="submit">
          {busy ? "Filing…" : "File it"}
        </Button>
        <Button className="button-quiet" disabled={busy} onClick={() => setOpen(false)}>
          Cancel
        </Button>
