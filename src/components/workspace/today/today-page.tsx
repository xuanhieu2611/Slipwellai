  ...
  return (
    <main className="workspace-page">
      <header className="page-intro">
        ...
        <p>Local time: {data.timezone}. Drag a task onto Top Three, or drag one back out. Slipwell never quietly carries yesterday’s priorities into today.</p>
      </header>
      <TodayBoard data={data as WorkspaceData} dueToday={dueToday} onCommand={command} today={today} topThree={topThree} />
          ...
          {data.routines.map((routine) => {
            ...
            return (
              <article className="record-card" key={routine.id}>
                <div>
                  <h3>{routine.name}</h3>
                  <p className="record-meta capitalize">{routine.period} · {resolved ? resolved.outcome : "Not yet checked"}</p>
                </div>
                {!resolved && (
                  <div className="record-actions">
                    <button className="button-base button-primary" onClick={() => safely(() => command({ action: "resolve_routine", routineId: routine.id, localDate: today, outcome: "completed" }), "Routine completed.")}>Complete</button>
                    <button className="button-base button-secondary" onClick={() => safely(() => command({ action: "resolve_routine", routineId: routine.id, localDate: today, outcome: "skipped" }), "Routine skipped.")}>Skip</button>
                  </div>
                )}
              </article>
            );
          })}
          {data.routines.length === 0 && <p className="empty-state">Add a routine from People & Notes when a repeated behavior belongs here, not in your task list.</p>}
        </div>
      </section>
      ...
      <section className="workspace-section">
        <div className="section-heading">
          ...
          <button className="button-base button-secondary" onClick={refreshAttention}>Refresh attention</button>
        </div>
        <div className="space-y-3">
          {coreSignals.map((signal) => (
            <SlippingSignalCard key={signal.id} signal={signal} data={data as WorkspaceData} command={command} resolveSignal={resolveSignal} />
          ))}
          {coreSignals.length === 0 && <p className="empty-state">No active task or project signals. Refresh attention to check meaningful activity against each cadence.</p>}
        </div>
      </section>
      <section className="workspace-section">
        <div className="section-heading">
        ...
        <div className="space-y-2">
          ...
          {data.captures.length === 0 && <p className="empty-state">Your captured thoughts will appear here.</p>}
        </div>
      </section>
    </main>
  );
