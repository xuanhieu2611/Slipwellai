...
export function SlippingSignalCard({
  ...
  const task =
    signal.entity_type === "task"
      ? data.tasks.find((candidate) => candidate.id === signal.entity_id)
      : undefined;
  const project =
    signal.entity_type === "project"
      ? data.projects.find((candidate) => candidate.id === signal.entity_id)
      : undefined;
  const defaultCadence =
    signal.entity_type === "project"
      ? (project?.slipping_cadence_days ?? 7)
      : (task?.slipping_cadence_days ?? 14);
  ...
  async function addNextAction(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (signal.entity_type === "project") payload.projectId = signal.entity_id;
        ...
        payload.domainId = task.domain_id;
        payload.projectId = task.project_id;
        payload.personId = task.person_id;
      }
  ...
  async function saveCadence(event: FormEvent<HTMLFormElement>) {
    ...
    if (!cadenceDays) {
      notify("Choose a cadence between 1 and 365 days.", "error");
      return;
    }
  ...
  async function pauseOrArchive() {
    try {
      if (signal.entity_type === "task")
        await command({ action: "delete_task", taskId: signal.entity_id });
      else await command({ action: "pause_project", projectId: signal.entity_id });
      await resolveSignal(
        signal.id,
        "dismissed",
        undefined,
        signal.entity_type === "task" ? "Task archived." : "Project paused.",
      );
    } catch (error) {
  ...
  return (
    <article className="record-card slipping-signal-enter">
      <div>
      ...
      <form className="inline-form" onSubmit={addNextAction}>
        <label className="sr-only" htmlFor={`next-action-${signal.id}`}>
          Next action
        </label>
        <input
          className="field-base"
          id={`next-action-${signal.id}`}
          name="title"
          placeholder="Add the next concrete action"
          disabled={busy}
        />
        <button className="button-base button-secondary" type="submit" disabled={busy}>
          {busy ? "Adding…" : "Add & mark attention"}
        </button>
      </form>
      <form className="inline-form" onSubmit={saveCadence}>
        <label className="sr-only" htmlFor={`cadence-${signal.id}`}>
          Attention cadence (days)
        </label>
        <input
          className="field-base"
          id={`cadence-${signal.id}`}
          type="number"
          name="cadenceDays"
          min={1}
          max={365}
      ...
      {(signal.entity_type === "task" || signal.entity_type === "project") && (
        <button className="button-base button-quiet" onClick={pauseOrArchive}>
          {signal.entity_type === "task" ? "Archive" : "Pause"}
        </button>
      )}
    </article>
  );
