...
export function NeedsAttention({ captureAttention }: { captureAttention: WorkspaceData["captureAttention"] }) {
  ...
  return (
    <section className="workspace-section">
      <div className="section-heading">
        ...
        {attention.length > 0 && <span className="tag tag--attention">{attention.length} waiting</span>}
      </div>
        ...
        {attention.map((capture) => (
          <article className="record-card" key={capture.id}>
            ...
            <div className="record-actions">
              <Link className="button-base button-secondary" href="/inbox" aria-label={`Resolve “${snippet(capture.original_text)}” in Inbox`}>
                Resolve in Inbox
              </Link>
            </div>
          </article>
        ))}
        {attention.length === 0 && <p className="empty-state">Nothing needs your attention. New captures land here only if they get stuck.</p>}
      </div>
    </section>
  );
}
