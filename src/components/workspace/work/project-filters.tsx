...
export type ProjectFilterState = {
  status: "current" | "completed" | "canceled" | "deleted" | "any";
};
...
export function filterProjects(projects: WorkspaceData["projects"], filters: ProjectFilterState) {
  return projects.filter((project) => {
    if (filters.status === "current")
      return !project.archived_at && ["planned", "active", "paused"].includes(project.status);
    if (filters.status === "completed")
      return !project.archived_at && project.status === "completed";
