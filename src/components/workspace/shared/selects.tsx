...
export function DomainColorPicker({ name = "color", defaultValue = DOMAIN_COLOR_PALETTE[0].value }: { name?: string; defaultValue?: string }) {
  ...
  const swatches = known ? DOMAIN_COLOR_PALETTE : [...DOMAIN_COLOR_PALETTE, { name: "Current", value: defaultValue }];
  return (
    <div className="field-label">
      <span>Color</span>
      <div className="color-swatch-grid">
        {swatches.map((swatch) => (
          <label className="color-swatch" key={swatch.value} style={{ background: swatch.value }} title={swatch.name}>
            <input aria-label={swatch.name} type="radio" name={name} value={swatch.value} defaultChecked={swatch.value.toUpperCase() === normalized} />
          </label>
        ))}
      </div>
    </div>
