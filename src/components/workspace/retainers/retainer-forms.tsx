...
export function RetainerEditForm({
  retainer,
  data,
  onCommand,
  onDone,
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await onCommand({
        ...
        cycleDay: formValue(form, "cycleDay"),
        clientPersonId: formValue(form, "clientPersonId"),
    ...
    <form className="form-grid" onSubmit={submit}>
      ...
      <label className="field-label">
        ...
        <input
          ...
          required
          maxLength={100}
          defaultValue={retainer.timezone}
        />
      </label>
      <label className="field-label">
        <span>Cycle day of month</span>
        <input
          className="field-base"
          type="number"
          name="cycleDay"
          ...
          max={31}
          required
          defaultValue={retainer.cycle_day}
        />
      </label>
      <DomainSelect domains={data.domains} defaultValue={retainer.domain_id ?? ""} />
      <PersonSelect
        people={data.people}
        name="clientPersonId"
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await onCommand({
        ...
        name: formValue(form, "name"),
        timezone: formValue(form, "timezone") ?? Intl.DateTimeFormat().resolvedOptions().timeZone,
        cycleDay: formValue(form, "cycleDay"),
        clientPersonId: formValue(form, "clientPersonId"),
        domainId: formValue(form, "domainId"),
    ...
    <form className="form-grid" onSubmit={submit}>
      <label className="field-label form-span">
        ...
        <input
          ...
          required
          maxLength={160}
          placeholder="Rivera Studio monthly retainer"
        />
      </label>
      <label className="field-label">
        <span>Timezone</span>
      ...
      <PersonSelect people={data.people} name="clientPersonId" />
