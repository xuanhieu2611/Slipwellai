...
} from "@/components/workspace/tasks/task-filters";
...
export function TasksPage({ data }: { data: TasksPageData }) {
  ...
  const slippingTaskIds = new Set(
    data.signals
      .filter((signal) => signal.entity_type === "task")
      .map((signal) => signal.entity_id),
  );
      ...
      <section className="workspace-section tasks-workspace">
        <div className="tasks-toolbar">
          <div
            ...
            data-active={taskView}
            role="group"
            aria-label="Task view"
          >
            <span className="task-view-switch-pill" aria-hidden />
            <button
              className={taskView === "week" ? "is-active" : undefined}
              type="button"
              aria-pressed={taskView === "week"}
            ...
            <button
              className={taskView === "planner" ? "is-active" : undefined}
              type="button"
              aria-pressed={taskView === "planner"}
              onClick={() => setTaskView("planner")}
            >
              <CalendarBlank aria-hidden size={17} weight="bold" />
              Planner
            </button>
            <button
              className={taskView === "list" ? "is-active" : undefined}
              type="button"
          ...
          <button
            className="button-base button-primary tasks-new-task"
            type="button"
        ...
        {taskView === "week" ? (
          <TaskWeekView tasks={calendarTasks} onCommand={command} today={today} data={fullData} />
        ) : null}
        {taskView === "planner" ? (
          <TaskPlanner
            tasks={calendarTasks}
