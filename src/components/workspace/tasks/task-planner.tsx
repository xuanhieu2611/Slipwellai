...
export function TaskPlanner({
  ...
  const selectedLabel =
    selectedDay === today
      ? `Today, ${calendarLabel(selectedDay, { weekday: "long", month: "long", day: "numeric" })}`
      : calendarLabel(selectedDay, {
          weekday: "long",
          month: "long",
          day: "numeric",
          year: "numeric",
    ...
    <div className="task-planner">
      <section className="task-calendar" aria-label="Task calendar">
        <div className="task-calendar-head">
          <div>
            <h2>{monthName}</h2>
            <p>Pick a day to see what belongs there.</p>
          </div>
          <div className="task-calendar-actions">
            {month.slice(0, 7) !== today.slice(0, 7) && (
              <button className="button-base button-quiet" type="button" onClick={returnToToday}>
                Today
              </button>
            )}
            <button
        ...
        <div className="task-calendar-grid">
          {days.map((day) => {
            ...
            const hasEarlierOpenWork =
              day < today &&
              tasks.some((task) => task.status === "open" && taskPlanningDate(task) === day);
            ...
            const className = [
              ...
              hasEarlierOpenWork && "has-earlier-work",
            ]
              .filter(Boolean)
              .join(" ");
            const dayLabel = calendarLabel(day, {
              weekday: "long",
              month: "long",
              day: "numeric",
              year: "numeric",
            ...
            return (
              <button
                ...
                type="button"
                key={day}
                aria-label={`${dayLabel}${count ? `, ${count} ${count === 1 ? "task" : "tasks"}` : ", no tasks"}`}
        ...
        <p className="task-calendar-note">
          Counts use a task&apos;s deferred date first, then due date, then scheduled date.
        </p>
      </section>

      <section className="task-agenda" aria-labelledby="selected-task-day">
        <div className="task-agenda-head">
          <div>
            <h2 id="selected-task-day">{selectedLabel}</h2>
            <p>
              {selectedTasks.length
                ? `${selectedTasks.length} ${selectedTasks.length === 1 ? "task" : "tasks"} on this day`
        ...
        <TaskList
          ...
          emptyText="No tasks on this day. Pick another date or open Unscheduled."
        />
        <details className="task-unscheduled">
          <summary>
            <span>
              <Tray aria-hidden size={17} />
              Unscheduled
            </span>
          ...
          <TaskList
            ...
            data={data}
            showTopThree={showTopThree}
            emptyText="Every task in this view has a date."
          />
        </details>
      </section>
    </div>
