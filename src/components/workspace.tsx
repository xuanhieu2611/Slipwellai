...
import { ArrowCounterClockwise, CalendarBlank, CaretDown, CaretLeft, CaretRight, DotsSixVertical, DotsThreeVertical, Flame, ListBullets, Plus, Rows, Star, Tray } from "@phosphor-icons/react";
import { DndContext, DragOverlay, PointerSensor, useDraggable, useDroppable, useSensor, useSensors, type DragEndEvent, type DragStartEvent } from "@dnd-kit/core";
import { SortableContext, arrayMove, useSortable, verticalListSortingStrategy } from "@dnd-kit/sortable";
import { CSS } from "@dnd-kit/utilities";
import { nextCycleMonth } from "@/lib/retainers";
import { activityEventLabel, calendarMonthGrid, calendarWeekDays, calendarWeekStart, isTaskOnDay, recurrenceLabel, retainerActivityEventLabel, routineCurrentStreak, routineHeatmapWeeks, shiftCalendarMonth, shiftCalendarWeek, taskDateLabel, taskPlanningDate, type RoutineHeatmapWeek, type WorkspaceData } from "@/lib/workspace";
import { useToast } from "@/components/ui/toast";
import { Dialog } from "@/components/ui/primitives";

type Surface = "today" | "tasks" | "work" | "retainers" | "people-notes" | "routines" | "search";
type CreateDialog = "domain" | "project" | "retainer" | "person" | "note" | null;
...
function NewTaskForm({ data, onCommand, onDone }: { data: WorkspaceData; onCommand: (command: Record<string, unknown>) => Promise<void>; onDone?: () => void }) {
  const notify = useToast();
...
function NewProjectForm({ data, onCommand, onDone }: { data: WorkspaceData; onCommand: (command: Record<string, unknown>) => Promise<void>; onDone?: () => void }) {
  const notify = useToast();
  const [busy, setBusy] = useState(false);
  const keyRef = useRef<string | null>(null);
  async function submit(event: FormEvent<HTMLFormElement>) {
...
function NewRetainerForm({ data, onCommand, onDone }: { data: WorkspaceData; onCommand: (command: Record<string, unknown>) => Promise<void>; onDone?: () => void }) {
  const notify = useToast();
  const [busy, setBusy] = useState(false);
  const keyRef = useRef<string | null>(null);
  async function submit(event: FormEvent<HTMLFormElement>) {
...
export function Workspace({ surface, data }: { surface: Surface; data: WorkspaceData }) {
  ...
  if (surface === "retainers") return <main className="workspace-page"><header className="page-intro page-intro--with-action"><div className="page-intro-text"><p className="eyebrow">Retainers</p><h1>Ongoing engagements, not projects with a finish line.</h1><p>Each cycle is a versioned, inspectable record. Incomplete work never silently disappears at rollover.</p></div><button className="button-base button-primary" onClick={() => setCreateDialog("retainer")}><Plus aria-hidden size={16} weight="bold" />New retainer</button></header>{createDialog === "retainer" && <Dialog title="New retainer" size="lg" onClose={() => setCreateDialog(null)}><NewRetainerForm data={data} onCommand={command} onDone={() => setCreateDialog(null)} /></Dialog>}<section className="workspace-section"><div className="section-heading"><div><h2>Retainers</h2><p className="section-note">Deliverables, cycles, and history</p></div><span className="tag">{filteredRetainers.length} shown</span></div><RetainerFilters filters={retainerFilters} onChange={setRetainerFilters} /><RetainerList retainers={filteredRetainers} data={data} onCommand={command} onCheckSlipping={checkRetainerSlipping} /></section></main>;

  if (surface === "people-notes") return <main className="workspace-page"><header className="page-intro page-intro--with-action"><div className="page-intro-text"><p className="eyebrow">People & Notes</p><h1>Context without turning everything into a task.</h1><p>People and reflective notes remain lightweight, private records.</p></div><div className="page-actions"><button className="button-base button-secondary" onClick={() => setCreateDialog("person")}><Plus aria-hidden size={16} weight="bold" />New person</button><button className="button-base button-primary" onClick={() => setCreateDialog("note")}><Plus aria-hidden size={16} weight="bold" />New note</button></div></header>{createDialog === "person" && <Dialog title="New person" onClose={() => setCreateDialog(null)}><form className="form-grid" onSubmit={(event) => submit(event, "create_person", { name: "name", context: "context", domainId: "domainId" }, "Person added.", () => setCreateDialog(null))}><label className="field-label form-span"><span>Name</span><input className="field-base" name="name" required maxLength={160} placeholder="Priya from Rivera Studio" /></label><label className="field-label form-span"><span>Context</span><input className="field-base" name="context" maxLength={1000} placeholder="Client lead, collaborator, or someone important" /></label><DomainSelect domains={data.domains} /><button className="button-base button-primary form-submit">Add person</button></form></Dialog>}{createDialog === "note" && <Dialog title="New note" size="lg" onClose={() => setCreateDialog(null)}><form className="form-grid" onSubmit={(event) => submit(event, "create_note", { title: "title", body: "body", domainId: "domainId", projectId: "projectId", personId: "personId", reviewOn: "reviewOn" }, "Note saved.", () => setCreateDialog(null))}><label className="field-label form-span"><span>Title</span><input className="field-base" name="title" required maxLength={280} placeholder="Rivera Studio call notes" /></label><label className="field-label form-span"><span>Note</span><textarea className="field-base min-h-32" name="body" maxLength={20000} placeholder="Keep the reflective content intact." /></label><DomainSelect domains={data.domains} /><label className="field-label"><span>Review on</span><input className="field-base" type="date" name="reviewOn" /></label><label className="field-label"><span>Project</span><select className="field-base" name="projectId" defaultValue=""><option value="">No project</option>{data.projects.filter((project) => !project.archived_at).map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><label className="field-label"><span>Person</span><select className="field-base" name="personId" defaultValue=""><option value="">No person</option>{data.people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><button className="button-base button-primary form-submit">Save note</button></form></Dialog>}<div className="workspace-columns"><section className="workspace-section"><div className="section-heading"><div><h2>People</h2><p className="section-note">Useful relationship context</p></div><span className="tag">{data.people.length}</span></div><div className="space-y-3">{data.people.map((person) => <article className="project-card" key={person.id}><div className="record-card"><div><h3>{person.name}</h3>{person.context && <p className="record-copy">{person.context}</p>}</div></div><PersonInteractions personId={person.id} data={data} onCommand={command} /></article>)}{data.people.length === 0 && <p className="empty-state">Add people when context helps, not as a CRM setup exercise.</p>}</div></section><section className="workspace-section"><div className="section-heading"><div><h2>Notes</h2><p className="section-note">Thinking worth keeping</p></div><span className="tag">{data.notes.length}</span></div><div className="space-y-3">{data.notes.map((note) => <article className="record-card" key={note.id}><div><h3>{note.title}</h3>{note.body && <p className="record-copy whitespace-pre-wrap">{note.body}</p>}<p className="record-meta">{note.review_on ? `Review ${note.review_on}` : "No review date"}</p></div></article>)}{data.notes.length === 0 && <p className="empty-state">Notes preserve thinking even when no action follows.</p>}</div></section></div></main>;

  if (surface === "routines") return <RoutinesPage data={data} today={today} onCommand={command} />;
...
function RoutinesPage({ data, today, onCommand }: { data: WorkspaceData; today: string; onCommand: (command: Record<string, unknown>) => Promise<void> }) {
  const notify = useToast();
  ...
  async function submitRoutine(event: FormEvent<HTMLFormElement>) {
    ...
    const form = event.currentTarget;
    try {
      await onCommand({ action: "create_routine", name: formValue(form, "name"), period: formValue(form, "period") });
      form.reset();
  ...
  async function resolve(routineId: string, outcome: "completed" | "skipped") {
    try {
      await onCommand({ action: "resolve_routine", routineId, localDate: today, outcome });
      notify(outcome === "completed" ? "Routine completed." : "Routine skipped.", "success");
    } catch (error) { notify(error instanceof Error ? error.message : "Could not save that change.", "error"); }
  }
  return <main className="workspace-page"><header className="page-intro page-intro--with-action"><div className="page-intro-text"><p className="eyebrow">Routines</p><h1>Habits worth keeping, tracked like it matters.</h1><p>Repeated behaviors stay separate from tasks: mark each one off as you go and watch the streak and history build.</p></div><button className="button-base button-primary" onClick={() => setCreateOpen(true)}><Plus aria-hidden size={16} weight="bold" />New routine</button></header>{createOpen && <Dialog title="New routine" onClose={() => setCreateOpen(false)}><form className="form-grid" onSubmit={submitRoutine}><label className="field-label"><span>Routine</span><input className="field-base" name="name" required maxLength={160} placeholder="Plan the day" /></label><label className="field-label"><span>Time of day</span><select className="field-base" name="period" defaultValue="anytime"><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="evening">Evening</option><option value="anytime">Anytime</option></select></label><button className="button-base button-primary form-submit">Add routine</button></form></Dialog>}<div className="space-y-4">{data.routines.map((routine) => <RoutineCard key={routine.id} routine={routine} completions={data.routineCompletions.filter((completion) => completion.routine_id === routine.id)} today={today} onResolve={resolve} />)}{data.routines.length === 0 && <p className="empty-state">Add a routine for a repeated behavior you want to keep an eye on - a workout, drinking water, anything that isn’t a one-off task.</p>}</div></main>;
}

function RoutineCard({ routine, completions, today, onResolve }: { routine: WorkspaceData["routines"][number]; completions: WorkspaceData["routineCompletions"]; today: string; onResolve: (routineId: string, outcome: "completed" | "skipped") => Promise<void> }) {
  ...
  return <article className="routine-card"><div className="routine-card-head"><div><h3>{routine.name}</h3><p className="record-meta capitalize">{routine.period} · {resolvedToday ? resolvedToday.outcome : "Not yet checked today"}</p></div><div className="routine-card-actions">{streak > 0 && <span className="routine-streak"><Flame aria-hidden size={14} weight="fill" />{streak} day{streak === 1 ? "" : "s"}</span>}{!resolvedToday && <div className="record-actions"><button className="button-base button-primary" onClick={() => onResolve(routine.id, "completed")}>Complete</button><button className="button-base button-secondary" onClick={() => onResolve(routine.id, "skipped")}>Skip</button></div>}</div></div><RoutineHeatmap weeks={weeks} /></article>;
}

function RoutineHeatmap({ weeks }: { weeks: RoutineHeatmapWeek[] }) {
  const cells = weeks.flatMap((week) => week.cells);
  return <div className="routine-heatmap"><div className="routine-heatmap-scroll"><div className="routine-heatmap-layout"><span className="routine-heatmap-corner" /><div className="routine-heatmap-months">{weeks.map((week) => <span key={week.start}>{week.monthLabel ?? ""}</span>)}</div><div className="routine-heatmap-weekdays">{ROUTINE_WEEKDAY_LABELS.map((label, index) => <span key={index}>{label}</span>)}</div><div className="routine-heatmap-grid">{cells.map((cell) => <span className="routine-heatmap-cell" data-outcome={cell.isFuture ? "future" : cell.outcome ?? "none"} data-today={cell.isToday ? "true" : undefined} key={cell.date} title={`${cell.date} · ${cell.outcome ?? (cell.isFuture ? "upcoming" : "no record")}`} />)}</div></div></div></div>;
}
