...
function VoiceRecorder({ done }: { done: () => void }) {
  ...
  function discardAfterFailure(message: string) {
    if (recording) URL.revokeObjectURL(recording.url);
    setRecording(null);
    idempotencyKeyRef.current = null;
    setState("failed");
  ...
  async function submitVoice() {
    ...
    try {
      ...
      formData.set("audio", recording.audio, "capture.webm");
      ...
      discardAfterFailure(error instanceof Error ? error.message : "Voice transcription failed. Please use text capture instead.");
    }
  }
  ...
  return <div className="mt-4 rounded-xl bg-[#f6f8f4] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">Voice capture</p><p className="text-xs text-[var(--ink-muted)]">{permission === "denied" ? "Microphone access is blocked" : state === "recording" ? "Recording…" : state === "paused" ? "Recording paused" : state === "transcribing" ? "Transcribing…" : "Record up to five minutes, then review the transcript."}</p></div>{state === "idle" ? <button type="button" disabled={capability !== "ready"} onClick={begin} className={`${control} border border-[var(--line)] bg-white`}>Record voice</button> : null}</div>{(state === "recording" || state === "paused") && <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={pauseOrResume} className={`${control} border border-[var(--line)] bg-white`}>{state === "paused" ? "Resume" : "Pause"}</button><button type="button" onClick={stop} className={`${control} bg-[var(--moss)] text-white`}>Stop recording</button></div>}{state === "preview" && recording && <div className="mt-3 space-y-3"><audio className="w-full" controls src={recording.url}>Your browser cannot play this recording.</audio><div className="flex flex-wrap gap-2"><button type="button" onClick={submitVoice} className={`${control} bg-[var(--moss)] text-white`}>Transcribe voice</button><button type="button" onClick={clearRecording} className={`${control} border border-[var(--line)] bg-white`}>Cancel recording</button></div></div>}{message && <p role="alert" className="mt-3 text-sm text-[#9b2c17]">{message}</p>}</div>;
}
