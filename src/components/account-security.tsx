...
export function AccountSecurity({ email, hasEmailPassword, hasGoogle, revokeAfterGoogle }: Props) {
  ...
  return (
    <section className="build-state max-w-2xl">
      <p className="eyebrow">Settings / Security</p>
      <h1>Your account stays in your hands.</h1>
      <p>
        Sign out of this browser when you are done, or verify your identity before ending every
        active session.
      </p>
      <div className="build-state-rule" />
      <section className="mt-7 rounded-lg border border-[var(--line)] bg-[var(--surface)] p-5">
        <h2 className="text-lg font-semibold">This browser</h2>
        <p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">Signed in as {email}.</p>
        <Button className="button-secondary mt-5" disabled={busy} onClick={signOutLocal}>
      ...
      <section className="mt-5 rounded-lg border border-[var(--line)] bg-[var(--surface)] p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">
          Download the records currently stored by the working prototype as a private JSON file.
          This direct export is best for normal-size accounts; large asynchronous exports,
          CSV/Markdown, media manifests, and account deletion are still being built.
        </p>
        <Button className="button-secondary mt-5" onClick={downloadExport}>
      ...
      <section className="mt-5 rounded-lg border border-[var(--line)] bg-[var(--surface)] p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">
          This ends active refresh sessions on your other browsers and devices, too.
        </p>
        {hasEmailPassword && (
          <form
          ...
          >
            <label className="field-label" htmlFor="security-password">
            ...
            <Button className="button-primary self-end" disabled={busy} type="submit">
              Sign out everywhere
            </Button>
          </form>
        )}
        {hasGoogle && (
          <Button className="button-secondary mt-5" disabled={busy} onClick={confirmWithGoogle}>
            Verify with Google and sign out everywhere
          </Button>
        )}
        {!hasEmailPassword && !hasGoogle && (
          <p className="mt-5 text-sm text-[var(--ink-muted)]">
            Use password recovery to add a sign-in method before managing all sessions.
          </p>
        )}
      </section>
...
function GoogleSessionRevocation() {
  ...
  return (
    <section className="build-state max-w-2xl">
      <p className="eyebrow">Settings / Security</p>
      <h1>Signing out everywhere…</h1>
      <p>Google verification is complete. We are ending active sessions now.</p>
    </section>
  );
}
