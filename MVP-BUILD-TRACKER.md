...
- [ ] Partial: record task creation for a project, milestone changes, explicit progress, pause, completion, and Slipping resolution as append-only activity; decisions and richer activity updates remain open. `resume_project`, `cancel_project`, `delete_project`, `restore_project`, and `delete_milestone` now record their own activity events (`resumed`/`canceled`/`deleted`/`restored`/`milestone_deleted`) the same way the existing project transitions do.
...
- [ ] Partial: schema input test covers template application IDs, and database uniqueness makes repeat application reconcile by project/template/version and source item; migration-backed idempotency, template-edit, completion, and cross-user tests remain open. Schema-level unit test coverage for `create_project` idempotency, `update_project`, and the new lifecycle commands now exists in `src/lib/workspace.test.ts`; migration-backed integration tests (mirroring `src/lib/workspace.integration.test.ts`'s task coverage) remain open.
